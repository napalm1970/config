...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("mail_fetcher.log"), logging.StreamHandler()],
)
...
class EmailFetcher:
    def __init__(self, provider, email_user, email_pass, save_dir="downloaded_emails"):
        ...
        if provider not in self.providers:
            raise ValueError(
                f"Неизвестный провайдер. Доступные: {list(self.providers.keys())}"
            )
    ...
    def fetch_emails(
        self, folder="INBOX", sender_filter=None, subject_filter=None, limit=10
    ):
        ...
        try:
            ...
            logging.info(
                f"Найдено писем: {len(email_ids)}. Обработка последних {limit}..."
            )
